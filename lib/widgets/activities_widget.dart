...
import 'activity_box_widget.dart';
import '../fitness_data.dart';
import 'graph.dart';
import '../hobbies_icons.dart';
...
class _ActivitiesWidgetState extends State<ActivitiesWidget>
  ...
  void initState() {
    ...
    _fadeAnimation =
        Tween<double>(begin: 0, end: 1).animate(widget._animationController);

    _activitiesOpenCloseAnimation =
        Tween<double>(begin: widget._screenHeight, end: 120.0).animate(
            CurvedAnimation(
                parent: widget._animationController, curve: Curves.decelerate));
  ...
  _buildTopLabels(List<GraphData> values) {
    return [
      values.elementAt(0),
      values.elementAt((values.length / 2).floor()),
  ...
  Widget getActivitiesWidget(context) {
    ...
    return Positioned(
      ...
      height: widget._screenHeight - 100,
      child: Stack(
        children: <Widget>[
          Container(
            decoration: BoxDecoration(
